class Timetable < ApplicationRecord
  ...
  def self.batch_update_or_create(timetable_params, open, close)
    ...
    bulk_insert_users_sql_arr = ["INSERT INTO timetables (location_id,date,open,close,closed,tbd,note,created_at,updated_at) VALUES #{params.join(", ")} ON DUPLICATE KEY UPDATE open = ?, close = ?, closed = ?, tbd = ?, updated_at= NOW()"] + values
