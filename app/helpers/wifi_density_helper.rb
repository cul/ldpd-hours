module WifiDensityHelper
  CACHE_DURATION = Rails.env.development? ? 10.seconds : 5.minutes

  def wifi_density_percentage(location)
    ...
    config_for_location = WIFI_DENSITY[location.code]
    return nil if config_for_location.nil? || !config_for_location.key?('high') || !config_for_location.key?('cuit_location_ids')

    Rails.cache.fetch('wifi_density_data-' + location.code, expires_in: CACHE_DURATION) do
      wifi_density_data_for_locations = Rails.cache.fetch('wifi_density_data', expires_in: CACHE_DURATION) do
      ...
      aggregated_locations.each do |location_id, location_data|
        total_client_count += location_data.fetch('client_count', 0).to_i
      end
  ...
  def fetch_raw_wifi_density_data
    if Rails.env.development? || Rails.env.test?
      JSON.parse(File.read(Rails.root.join('spec/fixtures/files/sample-wifi-density-response.json')))
    else
      JSON.parse(Net::HTTP.get(URI(root_url + "wifi-density.json")))
    end
  end

  def fetch_hierarchical_wifi_density_data
  ...
  def recursively_collect_children(location_data, children = {})
